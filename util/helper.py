...
from schemas.shared_models.models import AIMessage, HumanMessage, Message, MessageRole


class Trinity:
    @staticmethod
    def get_prompt_file(filename):
        return pkg_resources.files("prompts").joinpath(filename)
    ...
    @staticmethod
    def get_interaction_str(messages: List[Dict[str, Any]]) -> str:
        conversation_lines = []
        if messages:
            for msg in messages:
                conversation_lines.append(f"User: {msg.get('user_message','')}")
                conversation_lines.append(f"Assistant: {msg.get('llm_response','')}")
        conversation_str = "\n".join(conversation_lines)
        return textwrap.indent(conversation_str, "            ")
