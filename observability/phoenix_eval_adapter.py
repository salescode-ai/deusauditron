...
from src.deusauditron.app_logging.logger import logger
from src.deusauditron.config import TracingManager
...
def _boolify(value: Any) -> Optional[bool]:
    ...
    try:
        ...
        if isinstance(value, str):
            v = value.strip().lower()
            if v in ("true", "pass", "passed", "yes", "y"):  # common encodings
                return True
...
def log_rule_evaluation_span(
    *,
    ...
    """
    Emit a Phoenix/OpenInference span for a single rule evaluation.
    Safe no-op if tracing is disabled or tracer is unavailable.
    """
    try:
        ...
        # Generic fields commonly present in rule results
        passed = (
        ...
        with tracer.start_as_current_span(span_name):  # type: ignore
            ...
            try:
                ...
                # Never break evaluation flow for observability
                pass
    except Exception as e:
        # Fail-quiet logging to avoid any side effects
        try:
