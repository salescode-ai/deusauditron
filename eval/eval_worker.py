...
from schemas.autogen.references.granular_evaluation_result_schema import Category, GranularEvaluationResults
from schemas.autogen.references.llm_node_schema import LLMNodeConfig
from schemas.shared_models.models import AgentEvalRequest, AIMessage, HumanMessage, InteractionLog, Message, MiniInteractionLog
    ...
    async def _execute_node_evaluations(self, interaction_log: List[InteractionLog]) -> None:
        ...
        for node_block in node_blocks:
            ...
            if node_result.rule_results:
                ...
                serializable = [mi.model_dump() if hasattr(mi, 'model_dump') else mi.__dict__ for mi in node_block.mini_interaction_logs]
                node_evaluations.append(
                    GranularEvaluationResults(
                        category=Category.Node,
                        input_text=Trinity.get_interaction_str(serializable),
                        node=node_name,
